...
class AuthControllerIntegrationTest {
	...
	void request_withAlgNoneToken_returns401() throws Exception {
		...
		String payload =
				Base64.getUrlEncoder()
						.withoutPadding()
						.encodeToString(
								("{\"sub\":\"" + TEST_EMAIL + "\",\"role\":\"ROLE_CUSTOMER\"}")
										.getBytes(StandardCharsets.UTF_8));
		String algNoneToken = header + "." + payload + ".";

		// Act & Assert
		mockMvc.perform(get("/api/auth/me").header("Authorization", "Bearer " + algNoneToken))
				.andExpect(status().isUnauthorized());
	}
	...
	private String buildExpiredToken() {
		...
		// We create it via JwtService internals by using a very short-lived token that we
		// pre-compute.
		// Since we can't easily create an expired token without reflection, we use a hardcoded one
		// with a known-bad signature which will also be rejected (covers the "expired" path in
		// isTokenValid).
		String header =
				Base64.getUrlEncoder()
						.withoutPadding()
		...
		String payload =
				Base64.getUrlEncoder()
						...
						.encodeToString(
								("{\"sub\":\""
												+ TEST_EMAIL
												+ "\",\"role\":\"ROLE_CUSTOMER\",\"iat\":946684800,\"exp\":946684801}")
										.getBytes(StandardCharsets.UTF_8));
