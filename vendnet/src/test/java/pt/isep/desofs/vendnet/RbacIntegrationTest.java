...
class RbacIntegrationTest {
	...
	void customer_canAccess_machines_returns200() throws Exception {
		// Act & Assert
		mockMvc.perform(get("/api/machines").header("Authorization", "Bearer " + customerToken))
				.andExpect(status().isOk());
	}
	...
	void operator_canAccess_machines_returns200() throws Exception {
		// Act & Assert
		mockMvc.perform(get("/api/machines").header("Authorization", "Bearer " + operatorToken))
				.andExpect(status().isOk());
	}
	...
	void admin_canAccess_machines_returns200() throws Exception {
		// Act & Assert
		mockMvc.perform(get("/api/machines").header("Authorization", "Bearer " + adminToken))
				.andExpect(status().isOk());
	}
	...
	void operator_canAccess_sales_returns2xxOr404() throws Exception {
		...
		mockMvc.perform(
						get("/api/sales/machine/999999")
								.header("Authorization", "Bearer " + operatorToken))
				.andExpect(
						result -> {
							int status = result.getResponse().getStatus();
							if (status == 403) {
								throw new AssertionError("Expected 2xx or 4xx (not 403), got 403");
							}
						});
	}
